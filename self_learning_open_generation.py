...
def self_questioning_loop_open_generation(
    ...
    for iteration_idx in tqdm(range(num_iteration)):
        ...
        prompt2 = prompt_engineering_for_question_generation(topics_to_learn)
...
def self_questioning_loop_open_generation_batched(
        tokenizer, model, prompt_fn, extract_response_fn, batch_size, num_iteration=100,
    ...
    if not use_cache or not os.path.exists('storage/open_topics.pickle'):
        prompts = [prompt_engineering_for_topic_generation() for iteration_idx in range(num_iteration)]
        topics = generate_response_with_temperature(tokenizer, model, prompt_fn, extract_response_fn, prompts, temperature, pretrained_model_name, generation_config, batch_size)
        if baseline_mode:
            topics = ['AI, blockchain, cryptocurrency' for i in range(num_iteration)]
        with open('storage/open_topics.pickle', 'wb') as dumpfile:
            pickle.dump(topics, dumpfile, protocol=pickle.HIGHEST_PROTOCOL)
    else:
        with open('storage/open_topics.pickle', 'rb') as dumpfile:
            topics = pickle.load(dumpfile)
    print(f"Total number of topics: {len(topics)}")

    if not use_cache or not os.path.exists('storage/open_questions.pickle'):
        ...
        with open('storage/open_questions.pickle', 'wb') as dumpfile:
            pickle.dump(questions, dumpfile, protocol=pickle.HIGHEST_PROTOCOL)
    else:
        with open('storage/open_questions.pickle', 'rb') as dumpfile:
    ...
    if not use_cache or not os.path.exists('storage/open_passage_pool.pickle'):
        ...
        with open('storage/open_passage_pool.pickle', 'wb') as dumpfile:
            pickle.dump(passage_pool, dumpfile, protocol=pickle.HIGHEST_PROTOCOL)
    else:
        with open('storage/open_passage_pool.pickle', 'rb') as dumpfile:
            passage_pool = pickle.load(dumpfile)
    print(f"Total number of passages: {len(passage_pool)}")

    if not use_cache or not os.path.exists('storage/open_samples_pool.pickle'):
        ...
        with open('storage/open_samples_pool.pickle', 'wb') as dumpfile:
            pickle.dump(samples_pool, dumpfile, protocol=pickle.HIGHEST_PROTOCOL)
    else:
        with open('storage/open_samples_pool.pickle', 'rb') as dumpfile:
